...
rf.cat<-randomForest(cat_g3~. -G1 -G2 -G3 -ord_g3 -pf -failures -reason -Medu -Fedu,data = ftrain, mtry = 3, ntree=50, importance=TRUE) 
print(rf.cat)
importance(rf.cat)
varImpPlot(rf.cat)
...
```{r}
rf.cat1<-randomForest(cat_g3~failed + absences + sex + age + Walc + Fjob +goout + schoolsup + first_gen_college + guardian + Walc,data = ftrain, ntree=50, importance=TRUE) 
print(rf.cat1)
importance(rf.cat1)
varImpPlot(rf.cat1)
...
```
50.63% OOB estimate of error rate and 87.6% accuracy rate for the training data.
...
```{r}
rf.bin<-randomForest(pf~. -G1 -G2 -G3 -ord_g3 - cat_g3 -failures -reason -health -age -nursery,data = ftrain1,mtry=3, ntree=50, importance=TRUE) 
print(rf.bin)
importance(rf.bin)
varImpPlot(rf.bin)
rf.acc<- predict(rf.bin, ftrain1, type = 'class')
t<-table(predictions=rf.acc, actual=ftrain1$cat_g3)
t
sum(diag(t))/sum(t)
...
```{r}
rf.bin1<-randomForest(pf~failed + absences+ guardian + studytime + goout + schoolsup + first_gen_college + internet,data = ftrain1,mtry=3, ntree=50, importance=TRUE) 
print(rf.bin1)
importance(rf.bin1)
varImpPlot(rf.bin1)
rf.acc1<- predict(rf.bin1, ftrain1, type = 'class')
t<-table(predictions=rf.acc1, actual=ftrain1$cat_g3)
t
sum(diag(t))/sum(t)
